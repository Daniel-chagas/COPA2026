...
namespace Copa2026
{
    static class JogoCrud
    {
        ...
        public static void Alterar()
        {
            ...
            Console.WriteLine($"Atual: {Dados.NomeSelecao(j.IdTimeA)} x {Dados.NomeSelecao(j.IdTimeB)} | " +
                              $"Data: {j.Data} | Estádio: {Dados.NomeEstadio(j.IdEstadio)}");

            // Data — opcional: ENTER mantém
            Console.Write($"Nova data (dd/MM/yyyy) [ENTER manter '{j.Data}'] : ");
            string entradaData = Console.ReadLine()?.Trim() ?? "";
            if (entradaData.Length > 0)
            {
                // Valida o formato antes de aceitar
                if (DateTime.TryParseExact(entradaData, "dd/MM/yyyy",
                    System.Globalization.CultureInfo.InvariantCulture,
                    System.Globalization.DateTimeStyles.None, out _))
                {
                    Dados.jogos[idx].Data = entradaData;
                }
                else
                {
                    UI.Erro("Data inválida — mantida a data anterior.");
                }
            }

            // Estádio — opcional
            Console.Write("Novo ID de estádio (0 ou ENTER para manter) : ");
            string entradaEst = Console.ReadLine()?.Trim() ?? "";
            if (int.TryParse(entradaEst, out int idEst) && idEst > 0)
            {
                if (Dados.IndexEstadio(idEst) >= 0)
                    Dados.jogos[idx].IdEstadio = idEst;
                else
                    UI.Erro("Estádio não encontrado — mantido o anterior.");
            }
